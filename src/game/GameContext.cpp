...
bool GameContext::relicCanSpawn(RelicId relic, bool shopRoom) const
{
    ...
    case RelicId::BOTTLED_FLAME:
        return deck.getCountMatching([](auto c)
                                     { return c.getType() == CardType::ATTACK && c.getRarity() != CardRarity::BASIC; }, 1);

    case RelicId::BOTTLED_LIGHTNING:
        return deck.getCountMatching([](auto c)
                                     { return c.getType() == CardType::SKILL && c.getRarity() != CardRarity::BASIC; }, 1);

    case RelicId::BOTTLED_TORNADO:
        return deck.getCountMatching([](auto c)
                                     { return c.getType() == CardType::POWER; }, 1);
...
void GameContext::setupEvent()
{ // todo necronomicon event
    screenState = ScreenState::EVENT_SCREEN;
    ...
    case Event::FALLING:
    { // todo test and CANNOT BE BOTTLED
        int counts[3]{0, 0, 0};
    ...
    {
        if (disableMatchAndKeep)
        { // additional safeguard as it should also be prevented from spawning
            regainControl();
        }
...
void GameContext::afterBattle()
{
    ...
    case Room::MONSTER:
    {
        ...
        { // todo stolen gold actually comes first in the list
...
{
    auto list = gc.deck.getIdxsMatching([=](const auto &c)
                                        { return c.canUpgrade() && c.getType() == type; });
...
bool GameContext::obtainRelic(RelicId r)
{
    ...
    {
        info.toSelectCards.clear();
        deck.addMatchingToSelectList(info.toSelectCards, [](auto c)
...
CardId GameContext::returnColorlessCard(CardRarity rarity)
{
    ...
    for (const auto &c : colorlessCardPool)
    { // todo optimize
...
Event GameContext::getShrine(Random &eventRngCopy)
...
{
    if (idx1 < 0 || idx1 >= 12 || info.toSelectCards[idx1].card.id == CardId::INVALID || idx2 == idx1 || idx2 < 0 || idx2 >= 12 || info.toSelectCards[idx2].card.id == CardId::INVALID)
    {
        assert(false);
    }
...
void GameContext::chooseEventOption(int idx)
{
    ...
    case Event::CURSED_TOME:
    {
        ...
        case 5:
        {
            ...
            auto book = res == 0 ? RelicId::NECRONOMICON : (res == 1 ? RelicId::ENCHIRIDION : RelicId::NILRYS_CODEX);
...
void GameContext::chooseSelectCardScreenOption(int idx)
{
    ...
    if (!isLastCard)
    { // todo maybe refactor so dont remove card if last
        return;
    }
